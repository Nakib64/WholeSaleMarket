    ...
    if (user?.email) {
      axios
        ...
        .then((res) => {
          ...
          setFormData(res.data); // initial form data
          setLoading(false);
        })
                ...
                {isEditing && (
                  <div className="flex justify-end mt-8">
                    <button
                      ...
                      className="px-8 py-3 rounded-lg bg-primary text-white font-semibold hover:bg-primary/80 transition flex items-center gap-2"
                      disabled={updating}
                    >
                      {updating && <span className="loading loading-spinner loading-lg"></span>}
                      Save Changes
                    </button>
                  </div>
                )}
