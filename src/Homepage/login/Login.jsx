...
const Login = () => {
  ...
  const handleLogin = (e) => {
    ...
    login(email, password)
      .then((res) => {
        setUser(res.user);
        toast("Login Successful", { position: "top-right", autoClose: 2000, theme: "light", transition: Bounce });
        navigate(location.state ? location.state : "/");
      })
      .catch(() => {
        setError("Invalid email or password");
        toast("Invalid email or password", { position: "top-right", autoClose: 2000, theme: "light", transition: Bounce });
      });
  };
          ...
          <CardContent className="space-y-6">
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="flex justify-between text-sm text-gray-600">
                ...
                <p>
                  No account? <Link to="/register" className="text-indigo-600 font-semibold">Register</Link>
                </p>
              </div>

              <Button type="submit" className="w-full mt-4 py-3 bg-indigo-600 hover:bg-indigo-700 text-white">
                Login
              </Button>
            </form>
